...
设置了5个size为10000的数组，从而能从时间及消息输出两个维度来了解nextTick的执行情况。另外，额外增加了一个参数，即更新后的视图中<li>的数量，从这个数量，可以考察出同一事件循环中的nextTick执行情况。由运行结果图可以看出实际的输出与推导的输出结果相符合。
...
​    2、 vue DOM的视图更新实现，，使用到了ES6的Promise及HTML5的[MutationObserver](http://www.cnblogs.com/jscode/p/3600060.html)，当环境不支持时，使用setTimeout(fn, 0)替代。上述的三种方法，均为异步API。其中MutationObserver类似事件，又有所区别；事件是同步触发，其为异步触发，即DOM发生变化之后，不会立刻触发，等当前所有的DOM操作都结束后触发。